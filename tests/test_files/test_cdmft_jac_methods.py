...
clus_model = pyqcm.cluster_model(2, n_bath=4, name="clus_jac")
...
model = pyqcm.lattice_model(
    "Graphene_jac", clus, ((1, -1, 0), (2, 1, 0)), lattice=((1, -1, 0), (2, 1, 0))
)
...
def reset_bath() -> None:
    ...
    for name, val in initial_bath_params.items():
        model.set_parameter(name, val)


# --------------------------------
# NATIVE TRF + Scipy least squares
# --------------------------------
...
# -------------------------------------------------------
# BFGS + jac=True  (routes through scipy LM internally)
# -------------------------------------------------------
...
# ----------------------------------------------------------------
# L-BFGS-B + jac=True  (routes through scipy dogbox internally)
# ----------------------------------------------------------------
